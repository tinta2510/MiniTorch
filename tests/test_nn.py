...
def test_max(t: Tensor) -> None:
    ...
    for dim in range(len(t.shape)):
        # Apply minitorch max reduction along this dimension
        out = minitorch.max(t, dim)

        # Get expected shape after reducing `dim`
        ...
        # Manually compute the max values for comparison
        t_np = t.to_numpy()
        import numpy as np
        expected = np.max(t_np, axis=dim, keepdims=True)

        # Compare element-wise values
        out_np = out.to_numpy()
        assert np.allclose(out_np, expected, atol=1e-5)
